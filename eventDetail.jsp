...
<%
    ...
    PreparedStatement ps = conn.prepareStatement(
        "SELECT title, description, date, time, location, created_by,created_at FROM events WHERE event_id = ? AND created_by = ?");
    ps.setInt(1, eventId);
    ps.setInt(2, adminId);
  ...
  <script>
    function confirmDeleteAndRedirect() {
      if (confirm("Are you sure you want to delete this event?")) {
        window.location.href = "deleteEvent.jsp?event_id=<%= eventId %>";
      }
    }
  </script>
</head>
<body>
  <header>
    ...
    <div style="display:flex; gap:10px; flex-wrap:wrap;">
      <a class="button" href="editEvent.jsp?event_id=<%= eventId %>">Edit</a>
      <button class="button red" onclick="confirmDeleteAndRedirect()">Delete</button>
      <a class="button secondary" href="viewRegistrations.jsp?event_id=<%= eventId %>">View Registrations</a>
